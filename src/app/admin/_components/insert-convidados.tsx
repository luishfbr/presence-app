...
import { insertConvidados } from "@/app/_actions/server";
import type { Convidado } from "@/lib/types";

interface InsertProps {
  onInsert: () => void;
}

export function InsertConvidados({ onInsert }: InsertProps) {
  const { toast } = useToast()
  const [data, setData] = useState<Convidado[]>([]);
  ...
  const handleParseCSV = () => {
    ...
    reader.onload = ({ target }) => {
      ...
      Papa.parse(target.result as string, {
        ...
        skipEmptyLines: true,
        complete: (result) => {
          setData(result.data as Convidado[]);
        },
      });
    };
  ...
  const insertConvidadosFun = async () => {
    try {
      setLoading(true)
      const res = await insertConvidados(data)
      if (res?.status === 200) {
        toast({
          title: "Convidados inseridos com sucesso",
          description: "Os convidados foram inseridos com sucesso no banco de dados.",
        })
        setData([])
        onInsert()
      } else {
        toast({
          title: "Erro ao inserir convidados",
          description: "Ocorreu um erro ao inserir os convidados, tente novamente.",
          variant: "destructive"
        })
      }
    } catch (error) {
  ...
  return (
    <Dialog>
      <DialogTrigger asChild>
        <Button className="w-full">Inserir Convidados</Button>
      </DialogTrigger>
      <DialogContent className="sm:max-w-[525px]">
        ...
        <DialogFooter>
          <Button type="button" onClick={insertConvidadosFun} disabled={data.length === 0}>
